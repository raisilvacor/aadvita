...
def migrate(retries=8, delay=3.0):
    ...
    for attempt in range(1, retries + 1):
        try:
            print(f"[{attempt}/{retries}] Tentando conectar ao banco para migração eventos...")
            conn = psycopg.connect(database_url)
            conn.autocommit = True
            cur = conn.cursor()

            print("Executando: CREATE TABLE IF NOT EXISTS evento ...")
            cur.execute("""
            ...
            print("Executando: CREATE TABLE IF NOT EXISTS evento_foto ...")
            cur.execute("""
            ...
            print("Migração eventos executada com sucesso (Postgres).")
            cur.close()
            conn.close()
            return 0
